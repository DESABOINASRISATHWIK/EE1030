...
\begin{enumerate}
    ...
    \item 3
               \end{enumerate}
       \item
	       If the equation $a{\mod{Z}}^2 + \bar{\alpha}Z + \alpha \bar{Z} + d = 0$ represents a circle where a,d are real constants, then which of the following condition is correct?
		\begin{enumerate}
    \item $|\alpha|^2 - ad \neq 0$
...
\int \frac{(2x - 1) \cos\left(\sqrt{(2x - 1)^2 + 5}\right)}{\sqrt{4x^2 - 4x + 6}} \, dx $ is equal to (where \( c \) is a constant of integration):
    ...
    \item $ y \left(\frac{dy}{dx}\right)^2 - 2x \frac{dy}{dx} + y = 0 $
               \end{enumerate}
       \item
	       The real-valued function $ f(x) = \frac{\csc^{-1}(x)}{\sqrt{x - [x]}}$ where $[x]$ denotes the greatest integer less than or equal to $x$, is defined for all $x$ belonging to:
	       \begin{enumerate}
